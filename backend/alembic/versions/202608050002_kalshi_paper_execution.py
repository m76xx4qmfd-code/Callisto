...
def upgrade() -> None:
    ...
    op.execute(
        "DROP TRIGGER IF EXISTS trg_kalshi_paper_accounts_validate_journal ON kalshi_paper_accounts"
    )
    op.execute(
        "DROP TRIGGER IF EXISTS trg_kalshi_paper_decisions_validate_account_journal ON kalshi_paper_decisions"
    )
    op.execute(
